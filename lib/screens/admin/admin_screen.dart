...
// Halaman panel admin untuk mengelola data tempat, melihat statistik, dan mengelola kecamatan.
// Fungsi: Menampilkan daftar semua tempat dengan fitur pencarian, filter kategori, edit, hapus, dan lihat detail.
//          Menyediakan tab statistik (distribusi per kategori, total tempat, rating tertinggi).
//          Menyediakan tab untuk mengelola data kecamatan (polygon) melalui KecamatanManagementScreen.
// Informasi penting: Hanya dapat diakses oleh pengguna dengan role admin. Menggunakan TabController untuk tiga tab.
//                    Data tempat diambil dari TempatProvider. Operasi edit/hapus menggunakan SupabaseService.
...
import '../admin/kecamatan_management_screen.dart';

// Kelas AdminScreen adalah StatefulWidget karena memiliki TabController dan state pencarian/filter.
...
class _AdminScreenState extends State<AdminScreen>
  ...
  // Controller untuk tab (3 tab: Semua Tempat, Statistik, Kecamatan).
  late TabController _tab;
  // Teks pencarian tempat.
  String _search = '';
  // ID kategori yang dipilih untuk filter (null = semua kategori).
  int? _filterKat;
  // Status sedang menghapus (untuk menampilkan indikator loading).
  ...
  void initState() {
    ...
    // Catat bahwa panel admin dibuka (untuk logging).
    ErrorLogger.i('Admin panel opened');
  }

  @override
  Widget build(BuildContext context) {
    // Ambil data auth dan tempat dari provider.
    final auth = context.watch<AuthProvider>();
    final tp = context.watch<TempatProvider>();

    // Jika belum login atau bukan admin, tampilkan pesan akses ditolak.
    ...
    // Filter tempat berdasarkan pencarian dan kategori.
    final filtered = tp.allTempat.where((t) {
      final matchSearch = _search.isEmpty ||
          t.namaTempat.toLowerCase().contains(_search.toLowerCase());
      final matchKat = _filterKat == null || t.kategoriId == _filterKat;
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          // Tombol untuk membuka halaman Import/Export data.
          IconButton(
        ...
        bottom: TabBar(
          ...
          tabs: [
            // Tab 0: jumlah tempat ditampilkan dinamis.
            Tab(text: '📋 Semua Tempat (${tp.allTempat.length})'),
            const Tab(text: '📊 Statistik'),
            const Tab(text: '🗺️ Kecamatan'), // Tab baru untuk kelola kecamatan.
          ],
        ),
      ),
      body: TabBarView(
        controller: _tab,
        children: [
          // ── Tab 1: Data Management (daftar tempat) ────────────────────────
          Column(
            children: [
              ...
              // Baris jumlah tempat dan indikator deleting.
              Padding(
                padding:
          ...
          // Tab 3: Kecamatan Management (widget terpisah)
          const KecamatanManagementScreen(),
        ],
      ),
    );
  }

  // Menampilkan dialog konfirmasi hapus tempat.
  void _confirmDelete(BuildContext context, TempatModel t, TempatProvider tp) {
    showDialog(
      context: context,
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              ErrorLogger.i('Admin: deleting tempat id=${t.id}');
              ...
              if (ok) {
                ...
                ErrorLogger.e(
                    'Admin: failed to delete tempat id=${t.id}');
              }
            },
  ...
  // Menampilkan dialog edit tempat.
  void _showEditDialog(BuildContext context, TempatModel t, TempatProvider tp) {
    ...
    KategoriModel? selKat =
        tp.kategori.where((k) => k.id == t.kategoriId).firstOrNull;
    KecamatanModel? selKec =
        tp.kecamatan.where((k) => k.id == t.kecamatanId).firstOrNull;
    bool saving = false;
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                // Field Nama Tempat
                _dialogField(
                    namaCtrl, 'Nama Tempat', Icons.location_on_rounded),
              ...
              onPressed: saving
                  ...
                  : () async {
                      ...
                      if (ok) {
                        ...
                        ErrorLogger.e(
                            'Admin: failed to update tempat id=${t.id}');
                      }
                    },
              child: saving
                  ? const SizedBox(
  ...
  // Widget pembantu untuk membuat TextField dalam dialog edit.
  Widget _dialogField(
  ...
  @override
  void dispose() {
    _tab.dispose(); // Bebaskan resource TabController.
    super.dispose();
  }
}

// ── Admin Place Tile ───────────────────────────────────────────────
// Widget untuk menampilkan satu baris tempat di daftar admin.
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          // Informasi tempat (nama, kategori, kecamatan, rating, ID).
          Expanded(
            child: Padding(
...
// ── Statistics Tab ─────────────────────────────────────────────────
// Widget untuk menampilkan statistik (total tempat, rata-rata rating, distribusi per kategori, tempat dengan rating tertinggi).
  ...
  Widget build(BuildContext context) {
    ...
    final avgRating = tp.allTempat
        ...
        (tp.allTempat
            .where((t) => t.reviewRating != null)
            .length
            .clamp(1, 999)); // Hindari pembagian nol.
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // Kartu ringkasan: Total Tempat dan Rata-rata Rating.
          Row(
            children: [
              _StatBig(
                  label: 'Total Tempat',
          ...
          // Kartu tempat dengan rating tertinggi.
          if (topPlace != null)
            _TopPlaceCard(tempat: topPlace).animate().fade(delay: 100.ms),

          const SizedBox(height: 20),

          // Judul distribusi per kategori.
          const Text('Distribusi per Kategori', style: AppTextStyles.h3)
              .animate()
              .fade(delay: 150.ms),
          const SizedBox(height: 12),

          // Daftar statistik per kategori (baris progress bar).
          ...catStats.entries.toList().asMap().entries.map((e) {
            final i = e.key;
            final entry = e.value;
            final pct = totalTempat > 0 ? (entry.value / totalTempat) : 0.0;
            ...
            final catModel = tp.kategori.firstWhere(
                ...
                orElse: () => KategoriModel(id: 0, namaKategori: entry.key));
            ...
            return Container(
              ...
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      Text(catModel.icon, style: const TextStyle(fontSize: 18)),
                      const SizedBox(width: 8),
                      Text(entry.key,
                          style: const TextStyle(
                              fontFamily: 'Poppins',
...
// Kartu statistik besar (Total Tempat / Rata-rata Rating).
...
// Kartu untuk tempat dengan rating tertinggi.
