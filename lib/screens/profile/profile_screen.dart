...
// Halaman profil pengguna yang menampilkan informasi akun, statistik, dan menu pengaturan.
// Fungsi: Menampilkan foto profil (dapat diketuk untuk diperbesar dan diganti), nama, email, role,
//         statistik (total tempat, kategori, kecamatan), daftar tempat yang diunggah pengguna,
//         serta menu seperti riwayat dilihat, log error, dan logout.
// Informasi penting: Halaman ini memiliki dua tab (Profil dan Tempat Saya). Foto profil dapat diubah
//         melalui galeri, dengan upload ke Supabase storage. Data tempat yang diunggah diambil dari
//         Supabase berdasarkan user_id. Admin memiliki akses ke panel admin dan import/export data.
//         Untuk guest (belum login), menampilkan halaman berbeda dengan tombol login dan lihat tim.
...
// Kelas utama ProfileScreen (StatefulWidget karena memiliki TabController dan state tempat saya).
...
class _ProfileScreenState extends State<ProfileScreen>
  ...
  // Controller untuk tab (Profil dan Tempat Saya).
  late TabController _tabCtrl;
  // Daftar tempat yang diunggah oleh pengguna.
  List<TempatModel> _myPlaces = [];
  // Status loading untuk tempat yang diunggah.
  ...
  // Memuat tempat-tempat yang diunggah oleh pengguna yang sedang login.
  ...
  // Fungsi untuk mengganti foto profil (dipanggil dari tombol di header).
  Future<void> _changeProfilePhoto(AuthProvider auth) async {
    ...
    final fileName = await StorageService.uploadProfileImage(file);
    if (!mounted) return;
    if (fileName == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Gagal mengupload foto'), backgroundColor: AppColors.error));
      return;
    }
    
    // Hapus foto lama jika ada.
    final oldPhoto = auth.user?.photo;
    if (oldPhoto != null && oldPhoto.isNotEmpty) {
      await StorageService.deleteProfileImage(oldPhoto);
      if (!mounted) return;
    }
    
    // Update database dan session.
    final success = await AuthService.updateProfilePhoto(auth.user!.id, fileName);
    if (!mounted) return;
    if (success) {
      await auth.restoreSession();
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Foto profil berhasil diupdate'), backgroundColor: AppColors.success));
      setState(() {});
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Gagal update foto'), backgroundColor: AppColors.error));
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          // Admin memiliki akses ke panel admin.
          if (auth.isAdmin)
            IconButton(
      ...
      body: TabBarView(
        controller: _tabCtrl,
        children: [
          // Tab 1: Profil (dengan foto interaktif).
          _ProfileTab(
            auth: auth,
            tempat: tempat,
            onChangePhoto: () => _changeProfilePhoto(auth),
          ),
          // Tab 2: Tempat Saya (daftar tempat yang diunggah).
          _MyPlacesTab(
            places: _myPlaces,
...
// ── Tab Profil ─────────────────────────────────────────────
// Menampilkan informasi pengguna, foto profil (dapat diketuk untuk zoom), statistik, dan menu.
class _ProfileTab extends StatelessWidget {
  ...
  // Menampilkan foto profil dalam layar penuh dengan Hero animasi.
  void _showFullPhoto(BuildContext context, String? photoUrl, String name) {
  ...
  Widget build(BuildContext context) {
    ...
    return CustomScrollView(
      slivers: [
        // Header dengan foto profil (mirip lanyard) dan tombol ganti foto.
        SliverAppBar(
          expandedHeight: 280,
          pinned: true,
          automaticallyImplyLeading: false,
              ...
              child: SafeArea(
                child: Column(
                  ...
                  children: [
                    ...
                    // Foto profil (Hero untuk animasi saat diklik).
                    GestureDetector(
                      onTap: hasPhoto
                          ? () => _showFullPhoto(context, photoUrl, auth.user!.name)
                          : null,
                      child: Hero(
                        tag: 'profile_photo',
                        child: Container(
            ...
            child: Column(
              children: [
                ...
                _MenuSection(title: 'Akun', items: [
                  ...
                  // Menu Import/Export hanya untuk admin.
                  if (auth.isAdmin)
                    _MenuItem(
                ...
                _MenuSection(title: 'Informasi', items: [
                  ...
                  _MenuItem(
                    ...
                    onTap: () => launchUrl(
                      Uri.parse('https://github.com/hikmalt/DanLens-Mobile.git'),
  ...
  // Menampilkan bottom sheet riwayat tempat yang dilihat.
  ...
  // Menampilkan bottom sheet log error.
...
// ── Tab Tempat Saya ─────────────────────────────────────────────
// Menampilkan daftar tempat yang diunggah oleh pengguna, dengan opsi edit dan hapus.
class _MyPlacesTab extends StatelessWidget {
  ...
  // Dialog edit tempat (nama, detail, jalan, kontak).
...
// ── Sub-widget classes (StatCard, MenuSection, MenuItem, GuestProfile) ──
// Kartu statistik (total tempat, kategori, kecamatan).
...
// Grup menu dengan beberapa item.
...
// Satu baris menu (ikon, label, subtitle, trailing chevron).
...
// Halaman profil untuk pengguna yang belum login (guest).
