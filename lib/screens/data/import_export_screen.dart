// FILE: lib/screens/data/import_export_screen.dart
// Halaman untuk import dan export data tempat (tempat wisata, kuliner, dll).
// Fungsi: Menyediakan antarmuka bagi admin untuk mengekspor data tempat ke file PDF, Excel, atau SQL,
// serta mengimpor data dari file Excel (.xlsx, .xls) atau SQL (.sql) ke database.
// Informasi penting: Hanya dapat diakses oleh admin. Saat import, duplikat nama tempat akan dilewati.
// Menggunakan ExportService dan ImportService yang memanfaatkan Supabase.
...
// Kelas utama halaman import/export (StatefulWidget karena ada status loading).
...
class _ImportExportScreenState extends State<ImportExportScreen> {
  bool _exporting = false;         // Status sedang mengekspor.
  bool _importing = false;         // Status sedang mengimpor.
  String _status = '';             // Pesan status proses.
  // ignore: unused_field
  ImportResult? _lastImportResult; // Menyimpan hasil import terakhir (untuk keperluan debug).

  // ── EXPORT ──────────────────────────────────────────────────
  // Mengekspor data tempat ke format yang dipilih.
  Future<void> _export(String type) async {
    ...
    try {
      if (!mounted) return;
      final data = context.read<TempatProvider>().allTempat; // Ambil semua data tempat dari provider.
      if (data.isEmpty) {
      ...
      // Panggil service export sesuai format.
      switch (type) {
        case 'PDF':
  ...
  // ── IMPORT ──────────────────────────────────────────────────
  // Mengimpor data tempat dari file Excel atau SQL.
  Future<void> _import(String type) async {
    final auth = context.read<AuthProvider>();
    final tp = context.read<TempatProvider>(); // Ambil provider tempat untuk data existing.
    if (!auth.isLoggedIn) {
      _showSnack('Login terlebih dahulu untuk import', isError: true);
      return;
    }

    // Pilih file dari penyimpanan perangkat.
    ...
    try {
      final existing = tp.allTempat; // Data tempat yang sudah ada di database (untuk cek duplikat).
      ImportResult result;
      ...
      if (result.inserted > 0) {
        if (!mounted) return;
        await context.read<TempatProvider>().refresh(); // Refresh data setelah import.
      }
  ...
  // Menampilkan dialog hasil import (ringkasan sukses, duplikat, gagal, daftar error).
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // Indikator status (saat sedang export/import).
            ...
            // Baris tombol export (PDF, Excel, SQL).
            Row(
              children: [
                _ExportCard(
            ...
            Row(
              children: [
                ...
                // Badge "Login diperlukan" jika belum login.
                if (!auth.isLoggedIn)
                  Container(
            ...
            // Petunjuk format Excel yang diharapkan.
            Container(
              padding: const EdgeInsets.all(14),
              decoration: BoxDecoration(
            ...
            const SizedBox(height: 80), // Ruang di bawah agar tidak tertutup.
          ],
        ),
      ),
    );
...
// Kartu untuk tombol export (PDF, Excel, SQL).
...
// Kartu untuk tombol import (Excel atau SQL).
...
// Baris hasil import (untuk dialog).
...
// Baris format kolom (untuk petunjuk).
